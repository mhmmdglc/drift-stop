...
| 9 | ~~**Major (iOS)**~~ **Closed 2026-08-24 (`c1d15ca`).** | **iOS ads are misconfigured.** The fallback-to-`TestIds` branch was removed long ago (`resolveUnit` now serves nothing rather than a test ad), and on 2026-08-24 the iOS app and both iOS unit ids were created in the working publisher `pub-3817081931651779` and wired into `app.json` + `.env` + EAS `production`/`preview`. iOS `1.2.0 (9)` is the first build that carries real iOS ad ids. **Still not proven:** that an ad renders — the AdMob app is new and reads "İnceleme gerekli". |
